...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'aruliazmi') return res.sendFile(__path + '/views/eror.html')
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'aruliazmi')  return res.sendFile(__path + '/views/eror.html')
...
router.get('/darkjokes', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'aruliazmi')  return res.json(invalidKeynya)
       fetch(encodeURI(`https://raw.githubusercontent.com/aruliazmi/database-api/main/meme/darkjokes.json`))
        .then(response => response.json())
        .then(data => {
        var result = data;
        var result = data[Math.floor(Math.random() * data.length)];
